...
public class Application2 {
    ...
    * */
    ...
    // 배열의 할당을 할 때는 반드시 배열의 크기를 지정해주어야 한다.
//    iarr = new int[]; 크기를 지정해주지 않아서 에러가 발생

        iarr = new int[5];
        carr = new char[5]; // 타입도 맞춰줘야 한다.
        ...
        * -> 동일 객체인지 비교할 때 사용할 목적으로 많이 쓰이며, 동등 객체를 동일 객체 취급하기 위한 용도로 overring 해서 사용한다.
        * */
        ...
        darr = new double[30];
        // 배열의 사이즈를 변경한 것이 아니고,
        // 새로운 배열을 생성해서 그 주소값을 레퍼런스 변수에 덮어쓴 것이다.
        System.out.println("darr의 hashcode : " + darr.hashCode());
        System.out.println("darr의 길이 : " + darr.length);
