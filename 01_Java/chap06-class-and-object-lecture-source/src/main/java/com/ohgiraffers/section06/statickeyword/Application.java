...
public class Application {
    ...
    * static
    *
    * 정적 메모리 영역에 프로그램이 start 될 때 할당되는 키워드이다.
    * 인스턴스를 생성하지 않고도 사용할 클래스의 멤버(필드, 메소드)를 지정 가능하다.
    * 여러 인스턴스가 공유해서 사용할 목적의 공간이다.
    * 하지만 static 키워드의 남발은 유지보수와 추적이 힘든 코드를 작성하는 것을 피해야 할 방식
    ...
    * */
    ...
    // 두 필드가 가지고 있는 값 확인
        ...
        /*
        * 인스턴스 변수의 경우 sft1과 sft2 두개의 인스턴스가 서로 값을 공유하지 못하고
        * 인스턴스를 생성할 때마다 기본값이 0 으로 초기화가 되었다/
        * 하지만 static 필드의 경우 클래스 변수(static 붙은)가 프로그램 종료할 때까지
