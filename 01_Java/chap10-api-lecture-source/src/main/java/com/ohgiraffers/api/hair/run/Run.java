...
import com.ohgiraffers.api.hair.Customer;
...
public class Run {
    public static void main(String[] args) {
        ...
        Customer[] users = new Customer[temp.length]; //문자열 배열 temp의 길이와 동일한 길이의 Customer 객체 배열을 생성
        for (int i = 0; i < temp.length; i++) { // 문자열 배열을 반복하여 각 요소를 처리
            if(i == 0) continue; // 인덱스가 0인 경우에는 처리를 건너뛰고 다음 인덱스를 처리. 이 부분은 보통 CSV 파일의 첫 번째 줄이 헤더인 경우에 사용
            String[] userInfo = temp[i].split(","); // 현재 인덱스의 문자열을 쉼표(,)를 기준으로 분리하여 사용자 정보를 포함하는 배열화

            long id = Long.parseLong(userInfo[0]); // 사용자 ID를 문자열에서 long 값으로 변환.
            String firstName = userInfo[1];
            String lastName = userInfo[2];
            String email = userInfo[3];
            String gender = userInfo[4];
            // 나머지 사용자 정보들은 그대로 문자열 형태로 사용.
            // 예를 들어, String firstName = userInfo[1];은 사용자의 이름을 가져온다.
            ...
            // 예약 시간을 파싱합니다. 시간 형식이 "H:m"으로 지정되어 있다.
...
/*
정리 :
파싱은 일련의 문자열을 의미 있는 데이터로 변환하는 과정을 말합니다.
주로 데이터 형식을 변환하거나 특정 형식에 맞게 데이터를 추출하는 데 사용됩니다.

예를 들어, 날짜와 시간을 포함하는 문자열을 파싱할 때, 이 문자열을 날짜와 시간 객체로 변환하는 것이 일반적입니다.
이 과정에서는 문자열이 특정한 형식을 따르고 있어야 하며, 이 형식에 따라 파싱됩니다.

Java의 `DateTimeFormatter` 클래스를 사용하면 특정한 형식을 가진 문자열을 날짜나 시간 객체로 파싱할 수 있습니다.
이 클래스는 다양한 날짜와 시간 형식에 대한 패턴을 지원하며, `parse` 메서드를 사용하여 문자열을 해당 형식으로 파싱할 수 있습니다.

파싱은 문자열을 기반으로 하는 데이터를 응용 프로그램에서 사용할 수 있는 데이터 형식으로 변환하는 과정이기 때문에 매우 중요합니다.
*/
